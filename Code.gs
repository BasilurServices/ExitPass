...
function getAllPasses(body) {
  ...
  const offset    = parseInt(body.offset) || 0;
  const passSheet = getSheet("EXIT_PASSES");
  const passRows  = getAllRows(passSheet); // Get all to slice with offset
  const userMap   = buildUserMap();

  const allRows = passRows.reverse();
...
/**
 * Updates user profile details (email and phone).
 */
function updateUserProfile(body) {
  ...
  for (const sheetName of USER_SHEETS) {
    ...
    for (let i = 1; i < rows.length; i++) {
      if (normalizeUserId(rows[i][USER_COLS.user_id - 1]) === userId) {
        if (email !== undefined) {
          sheet.getRange(i + 1, USER_COLS.email).setValue(email);
        }
        if (phone !== undefined) {
